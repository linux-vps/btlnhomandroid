            ...
            try {
                ...
                if (pdfUri != null) {
                    ...
                    if (outputStream != null) {
                        ...
                        document.add(new Paragraph("BÁO CÁO CHI TIÊU").setBold().setFontSize(16));
                        document.add(new Paragraph("\nCHI TIẾT CHI TIÊU:").setBold());

                        for (Expense expense : expenses) {
                            document.add(new Paragraph("- " + expense.getTitle() + ": " + expense.getAmount() + " " + group.getCurrency() + expense.getDate().toString()));
                        }

                        document.add(new Paragraph("\n\nGỢI Ý CHIA TIỀN:").setBold());
                        for (DetailsListObject detail : detailsList) {
                            document.add(new Paragraph("- " + detail.getOwerName() + " trả " + detail.getOwedAmount() + " " + group.getCurrency() + " cho " + detail.getOweeName()));
                        }
